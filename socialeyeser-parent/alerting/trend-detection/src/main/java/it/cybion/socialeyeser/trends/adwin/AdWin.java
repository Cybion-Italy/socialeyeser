 ...
 */
package it.cybion.socialeyeser.trends.adwin;
...
public class AdWin { // extends Estimator {
    ...
    public void compressBuckets() {
        ...
        do {
            ...
            if (k == MAXBUCKETS + 1) {
                ...
                nextNode.insertBucket(cursor.Total(0) + cursor.Total(1), cursor.Variance(0)
                        + cursor.Variance(1) + incVariance);
                BucketNumber++;
            ...
            while (blnReduceWidth) // Diference
            {
                ...
                do {
                    for (int k = 0; k <= (cursor.bucketSizeRow - 1); k++) {
                        ...
                        u2 = cursor.Total(k);
                        if (n0 > 0)
                            v0 += cursor.Variance(k) + (double) n0 * n2 * (u0 / n0 - u2 / n2)
                                    * (u0 / n0 - u2 / n2) / (n0 + n2);
                        if (n1 > 0)
                            v1 -= cursor.Variance(k) + (double) n1 * n2 * (u1 / n1 - u2 / n2)
                                    * (u1 / n1 - u2 / n2) / (n1 + n2);
    ...
    private boolean blnCutexpression(int n0, int n1, double u0, double u1, double v0, double v1,
            double absvalue, double delta) {
